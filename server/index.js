...
const issueSchema = z.object({
  ...
  ward_id: z.coerce.number(),
  has_citizen_consent: z.boolean().optional().default(false),
...
app.post('/api/issues', async (req,res) => {
  try{
    ...
    const insertQuery = `
      INSERT INTO issues (title,description,category,latitude,longitude,image_url, is_anonymous,ward_id, sla_deadline, has_citizen_consent, consent_timestamp, privacy_policy_version, email)
    ...
    const values = [
      ...
      validated.has_citizen_consent, validated.consent_timestamp, validated.privacy_policy_version || 'v1.0', validated.email || null
    ];
...
app.post('/api/issues/:id/escalate', async (req,res) => {
  try{
    const id = paramIdSchema.parse(req.params.id);
    ...
    if (err instanceof z.ZodError) return res.status(400).json({ error: 'Invalid issue ID format' });
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.patch('/api/issues/:id', authenticate, isAdmin, async (req,res) => {
  try{
    ...
    const current = await db.query('SELECT status, email, title FROM issues WHERE id = $1', [id]);
    if(current.rows.length === 0) return res.status(404).json({error: 'Issue not found'});
    const issueData = current.rows[0];
    ...
    if(validated.status === 'resolved' && issueData.email){
      console.log(`\n========================================`);
      console.log(`MOCK EMAIL NOTIFICATION`);
      console.log(`To: ${issueData.email}`);
      console.log(`Subject: Update on your issue #${id} - ${issueData.title}`);
      console.log(`Body: Great news! Your reported issue has been successfully resolved. Thank you for making your community better.`);
      console.log(`========================================\n`);
    }
